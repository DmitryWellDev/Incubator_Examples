import React, {useState} from "react";
import styles from './uncontroledRating.module.css'
import {RatingValueType} from "../Rating/Rating";

type SelfControledRating = {
    defaultValue?: RatingValueType
    onChange: (value: RatingValueType) => void
}

function SelfControledRating(props: SelfControledRating) {

    let [value, setValue] = useState<RatingValueType>(props.defaultValue ? props.defaultValue : 0)

    return (
        <div className={styles.item_wrap}>
            <Star selected={value > 0} setValue={() => {setValue(1); props.onChange(1)}}/>
            <Star selected={value > 1} setValue={() => {setValue(2); props.onChange(2)}}/>
            <Star selected={value > 2} setValue={() => {setValue(3); props.onChange(3)}}/>
            <Star selected={value > 3} setValue={() => {setValue(4); props.onChange(4)}}/>
            <Star selected={value > 4} setValue={() => {setValue(5); props.onChange(5)}}/>
        </div>
    )
}

type StarPropsType = {
    selected: boolean
    setValue: () => void
}

function Star(props: StarPropsType) {

    const setValue = () => {
        props.setValue()
    }

return <div>
    <span onClick={setValue} className={styles.item}>{props.selected ? <b>star</b> : 'star'}</span>
</div>

}

export default SelfControledRating